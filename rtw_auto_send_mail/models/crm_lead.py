...
class rtw_crm(models.Model):
    ...
    def _send_mail(self):
        ...
        itoshima_operation_type = self.env['stock.picking.type'].search([('warehouse_id','=',itoshima_warehouse.id),('sequence_code','=','IN')])

        if itoshima_operation_type:
            itoshima_stock_picking = self.env['stock.picking'].search([('picking_type_id','=',itoshima_operation_type.id)])
            for index , picking in enumerate(itoshima_stock_picking):
                if index == 0:
                    body+= '<br>'
        ...
        body = body + '<br>' + '糸島入荷一覧URL' + '<br>' + f'https://rtw.cloudsv.mydns.jp/web?#action={itoshima_filter_action_id}&active_id={itoshima_operation_type.id}&model=stock.picking&view_type=list&cids=1&menu_id=244' + '<br>' + '<br>' + '※本メールはシステム自動配信です、返信は無効となります。'
