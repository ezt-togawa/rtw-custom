        ...
        for  index,stock_picking in enumerate(lines):
            ...
            if stock_moves :
                for line in stock_moves:
                    ...
                    lot = lot.rstrip("\n")
                    sheet.merge_range(row_inside, 5, row_inside, 6, lot, format_size10)
                    
                    sheet.write(row_inside, 3, prod_tmpl, format_left)
                    sheet.write(row_inside , 7, package, format_wrap)
                    row_inside += 1
    
                merge_to = row_inside - 1
                if row_start == merge_to:
                    sheet.write(row_start , 0, index + 1, format_wrap)
                    sheet.write(row_start , 1, order_number, format_wrap)
                    sheet.write(row_start , 2, sale_title, format_wrap)
                    sheet.write(row_start , 4, confirmed_shipping_date, format_wrap)
                else:
                    sheet.merge_range(row_start , 0, merge_to, 0, index + 1, format_wrap)
                    sheet.merge_range(row_start , 1, merge_to, 1, order_number, format_wrap)
                    sheet.merge_range(row_start , 2, merge_to, 2, sale_title, format_wrap)
                    sheet.merge_range(row_start , 4, merge_to, 4, confirmed_shipping_date, format_wrap)
                ...
                if row_start == merge_to:
                    ...
                    sheet.write(row_start , 1, order_number, format_wrap)
                    sheet.write(row_start , 2, sale_title, format_wrap)
                    sheet.write(row_inside, 3, "", format_left)
                    sheet.write(row_start , 4, confirmed_shipping_date, format_wrap)
                    sheet.merge_range(row_start, 5, row_start, 6, "", format_size10)
                    sheet.write(row_inside , 7, "", format_wrap)
                else:
                    sheet.merge_range(row_start , 0, merge_to, 0, index + 1, format_wrap)
                    sheet.merge_range(row_start , 1, merge_to, 1, order_number, format_wrap)
                    sheet.merge_range(row_start , 2, merge_to, 2, sale_title, format_wrap)
                    sheet.merge_range(row_start , 3, merge_to, 3, "", format_wrap)
                    sheet.merge_range(row_start , 4, merge_to, 4, confirmed_shipping_date, format_wrap)
                    sheet.merge_range(row_start , 5, merge_to, 6, "", format_wrap)
