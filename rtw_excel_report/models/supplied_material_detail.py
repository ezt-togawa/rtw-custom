...
class productSpec(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        format_title = workbook.add_format({'align': 'center','valign': 'vcenter','text_wrap':True,'bg_color':"yellow",'font_size':18,'bold':True})
        format_table_yellow = workbook.add_format({'align': 'center','valign': 'vcenter','border':1,'top':1,'text_wrap':True,'bg_color':"yellow"})
        format_bold = workbook.add_format({'align': 'center','valign': 'vcenter','text_wrap':True,'bold':True})
        format_bold_left = workbook.add_format({'align': 'left','valign': 'vcenter','text_wrap':True,'bold':True})
        format_add_user_login = workbook.add_format({'align': 'left','valign': 'vcenter','text_wrap':True,'font_size':9})
        ...
        sheet.write(7,0, "届先名", format_bold)
        sheet.write(9,0, "月日", format_bold)
        ...
        sheet.merge_range(6,6,6,7,"株式会社 リッツウェル", format_bold_left)
        ...
        for  index,stock_picking in enumerate(lines):
            ...
            if stock_picking.confirmed_shipping_date:
                confirmed_shipping_date="平成 " + str(stock_picking.confirmed_shipping_date).split("-")[0] + " 年 " + str(stock_picking.confirmed_shipping_date).split("-")[1] + " 月 " + str(stock_picking.confirmed_shipping_date).split("-")[2]

            sheet.merge_range(7,1,7,2,company_name, format_bold_left)
            sheet.merge_range(9,1,9,2,confirmed_shipping_date, format_bold_left)
            ...
            if stock_picking.product_id.product_tmpl_id.width or stock_picking.product_id.product_tmpl_id.width == 0:
                prod_width = stock_picking.product_id.product_tmpl_id.width
            ...
            if(len(stock_move_lines)==1):
                ...
                sheet.write(row_remember , 7, "W*" + str(prod_width), format_wrap)
                ...
                sheet.merge_range(row_remember , 7, merge_to,7,"W*" + str(prod_width), format_wrap)
