...
class ReportMrpExcel(models.AbstractModel):
    """
        【発注書】Excelレポート出力ロジック
        レポートID: rtw_excel_report.purchase_order_sheet_xls
        用途: 製造オーダーより発注書のExcelを作成する
    """
    ...
    def generate_xlsx_report(self, workbook, data, mrp_data):
        ...
        format_remark_note = workbook.add_format({'align': 'left', 'valign': 'top', 'text_wrap':True, 'font_name': font_name, 'font_size':13})
        format_address = workbook.add_format({'align': 'left', 'valign': 'top', 'text_wrap':True, 'font_name': font_name, 'font_size':10.5})
        format_table = workbook.add_format({'align': 'center','valign': 'vcenter','bg_color': '#999999', 'font_name': font_name,'font_size':13, 'color':'white','bold':True})
        format_table_left = workbook.add_format({'align': 'left','valign': 'vcenter','bg_color': '#999999', 'font_name': font_name,'font_size':12, 'color':'white','bold':True})
        format_lines_13 = workbook.add_format({'align': 'center', 'valign': 'vcenter', 'text_wrap':True, 'font_name': font_name, 'font_size':13, 'bottom':1})
        format_lines_16 = workbook.add_format({'align': 'right', 'valign': 'vcenter', 'text_wrap':True, 'font_name': font_name, 'font_size':16, 'bottom':1})
        ...
        format_lines_10_left = workbook.add_format({'align': 'left', 'valign': 'vcenter', 'text_wrap':True, 'font_name': font_name, 'font_size':13, 'bottom':1})
        format_lines_10_right = workbook.add_format({'align': 'right', 'valign': 'vcenter', 'text_wrap':True, 'font_name': font_name, 'font_size':13, 'bottom':1})
        ...
        def apply_sheet_settings(sheet):
            ...
            sheet.set_margins(left=0.2, right=0.2, top=0.2, bottom=0.2) # 左,右,上,下 の余白（インチ指定:cmは2.54倍）
        ...
        if allow_print:
            for mrp in mrp_data[0]:
                ...
                if mrp.sale_order and mrp.sale_order.check_oversea:
                ...
                if mrp.picking_type_id.warehouse_id.partner_id:
                    if mrp.lang_code == "ja_JP":
                        sheet.write(3, 0, mrp.picking_type_id.warehouse_id.partner_id.name + _(' 御中'), format_text_14)
                    else:
                        sheet.write(3, 0, "Dear " + mrp.picking_type_id.warehouse_id.partner_id.name, format_text_14)
                ...
                if mrp.address_ship == "倉庫":
                        ...
                        sheet.write(11, 1, mrp.mrp_choose_option_find_warehouse_company_name if mrp.mrp_choose_option_find_warehouse_company_name else '', format_text_13)
                        sheet.write(12, 1, mrp.mrp_choose_option_find_warehouse_address if mrp.mrp_choose_option_find_warehouse_address else '', format_text_13)
                        sheet.write(13, 1, mrp.mrp_choose_option_find_warehouse_phone if mrp.mrp_choose_option_find_warehouse_phone else '', format_text_13)
                sheet.write(12, 0, _("住所"), format_text) 
                sheet.write(13, 0, _("TEL"), format_text)
                if mrp.address_ship and mrp.address_ship == "直送":
                    sheet.write(11, 1, mrp.mrp_production_so_id.shipping_destination_text  if mrp.mrp_production_so_id.shipping_destination_text  else '', format_text_13)
                    sheet.set_row(12, 0)
                    sheet.set_row(13, 0)
                elif mrp.address_ship and mrp.address_ship == "デポ１":
                        sheet.write(11, 1, mrp.mrp_production_so_id.waypoint.last_name if mrp.mrp_production_so_id.waypoint.last_name else '', format_text_13)
                        sheet.write(12, 1, mrp.mrp_production_so_id.sale_order_waypoint_address if mrp.mrp_production_so_id.sale_order_waypoint_address else '', format_text_13)
                        sheet.write(13, 1, mrp.mrp_production_so_id.waypoint.phone if mrp.mrp_production_so_id.waypoint.phone else '', format_text_13)
                elif mrp.address_ship and mrp.address_ship == "デポ２":
                        sheet.write(11, 1, mrp.mrp_production_so_id.waypoint_2.last_name if mrp.mrp_production_so_id.waypoint.last_name else '', format_text_13)
                        sheet.write(12, 1, mrp.mrp_production_so_id.sale_order_waypoint_address if mrp.mrp_production_so_id.sale_order_waypoint_address else '', format_text_13)
                        sheet.write(13, 1, mrp.mrp_production_so_id.waypoint_2.phone if mrp.mrp_production_so_id.waypoint.phone else '', format_text_13)
                sheet.set_row(14, 0)
                sheet.set_row(15, 0)
                
                sheet.write(8, 1, mrp.sale_order.title if isLinkeSale and mrp.sale_order.title else '', format_text_14)
                sheet.write(9, 1, mrp.sale_order.sale_order_info_cus if isLinkeSale and mrp.sale_order.sale_order_info_cus else '', format_text_13)
            ...
            for index,mrp in enumerate(mrp_data):
                ...
                sheet.merge_range(row, 8, row + height, 8, mrp.mrp_product_product_qty, format_lines_16)
                
                if mrp.product_id.product_tmpl_id.uom_id.name:
                    sheet.merge_range(row, 9, row + height, 9, mrp.product_id.product_tmpl_id.uom_id.name, format_lines_13)
                else:
                    sheet.merge_range(row, 9, row + height, 9, "", format_lines_13)
            ...
            for mrp in mrp_data[0]:
                ...
                if mrp.is_child_mo:
                    ...
                    if mrp.mrp_mo_date:
                        ...
                        so = self.env["sale.order"].search([('name', '=', mrp.sale_reference)], limit=1)
                        if so:
                            sheet.write(6, 1, so.mo_shiratani_entry_date if so.mo_shiratani_entry_date else '',
                                        format_text_date)
                elif mrp.picking_type_id.warehouse_id and mrp.picking_type_id.warehouse_id.name != "糸島工場":
                    so = self.env["sale.order"].search([('name', '=', mrp.sale_reference)], limit=1)
                    if so:
                        if mrp.address_ship == '直送':
                            sheet.write(6, 1, mrp.mo_estimated_shipping_date if mrp.mo_estimated_shipping_date else '',
                                        format_text_date)
                        elif mrp.address_ship == '倉庫' and mrp.storehouse_id.name == '白谷運輸':
                            sheet.write(6, 1, mrp.mo_shiratani_date if mrp.mo_shiratani_date else '', format_text_date)
                        elif mrp.address_ship == '倉庫' and mrp.storehouse_id.name != '白谷運輸':
                            ...
                            sheet.write(6, 1, so.mo_warehouse_arrive_date if so.mo_warehouse_arrive_date else '',
                                        format_text_date)
                        elif mrp.address_ship == 'デポ２':
                            sheet.write(6, 1, so.mo_warehouse_arrive_date_2 if so.mo_warehouse_arrive_date_2 else '',
                                        format_text_date)
                elif isLinkeSale and mrp.sale_order.sipping_to == 'direct':
                    sheet.write(6, 1,
                                mrp.sale_order.sale_order_preferred_delivery_date if mrp.sale_order.sale_order_preferred_delivery_date else '',
                                format_text_date)
                elif isLinkeSale:
                    sheet.write(6, 1,
                                mrp.sale_order.so_warehouse_arrive_date_has_day if mrp.sale_order.so_warehouse_arrive_date_has_day else '',
                                format_text_14)
                elif not isLinkeSale:
                    sheet.write(6, 1, mrp._format_date(mrp.estimated_shipping_date,
                                                       mrp.lang_code) if mrp.estimated_shipping_date else '',
                                format_text_date)
                ...
                if mrp.mrp_production_so_id.sipping_to == 'direct':
                    ...
                    if mrp.address_ship and mrp.address_ship == "デポ/直送":
                        sheet.write(11, 1, mrp.sale_order.sale_order_waypoint_name if isLinkeSale and mrp.sale_order.sale_order_waypoint_name else '', format_text_13)
                        sheet.write(12, 1, mrp.sale_order.sale_order_waypoint_address if isLinkeSale and mrp.sale_order.sale_order_waypoint_address else '', format_text_13)
                        sheet.write(13, 1, mrp.sale_order.waypoint.phone if isLinkeSale and mrp.sale_order.waypoint.phone else '', format_text_13)
                    else:
                        sheet.write(11, 1, mrp.mrp_choose_option_find_warehouse_company_name if mrp.mrp_choose_option_find_warehouse_company_name else '', format_text_13)
                        sheet.write(12, 1, mrp.mrp_choose_option_find_warehouse_address if mrp.mrp_choose_option_find_warehouse_address else '', format_text_13)
                        sheet.write(13, 1, mrp.mrp_choose_option_find_warehouse_phone if mrp.mrp_choose_option_find_warehouse_phone else '', format_text_13)
