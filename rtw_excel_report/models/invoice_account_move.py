...
class ReportMrpExcel(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, so_data):
        ...
        for index,so in enumerate(so_data):
            ...
            # y,x
            sheet.write(1, 1, "御請求書", format_sheet_title) 
            ...
            sheet.write(6, 0, "下記の通り、ご請求申し上げます。", format_text) 
            sheet.write(8, 0, "件名", format_text_14_border) 
            ...
            sheet.write(4, 5, "お振込先", format_text_right) 
            ...
            sheet.write(7, 5, "納品日", format_text_right) 
            sheet.write(8, 5, "納品場所", format_text_right) 
            ...
            sheet.merge_range(9, 6, 11, 8, so.sale_order.sale_order_special_note[:120] if so.sale_order.sale_order_special_note else '', format_note) 

            sheet.merge_range(0, 11, 0, 12, so.acc_move_current_date if so.acc_move_current_date else '' , format_date) 
            sheet.merge_range(2, 11, 8, 12, so.sale_order.sale_order_hr_employee_invoice if so.sale_order.sale_order_hr_employee_invoice else '' , format_address) 

            sheet.write(13, 9, '定価合計: ' + str(so.acc_move_total_list_price) if so.acc_move_total_list_price else '', format_text_right) 
            sheet.write(13, 12, '販売価格合計: ' + str(so.acc_move_amount_untaxed) if so.acc_move_amount_untaxed else '', format_text_right) 
            ...
            if so.invoice_line_ids:
                ...
                for line in so.invoice_line_ids:
                        ...
                        sheet.merge_range(row, 10, row + merge_line, 10, line.acc_line_discount if line.acc_line_discount else '' , format_lines_10) 
