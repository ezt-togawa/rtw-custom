...
class StockPickingDelivery(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, so_data):
        ...
        format_lines_12_left = workbook.add_format({'align': 'left','valign': 'vcenter', 'text_wrap':True,'font_name': font_name,'font_size':12,'bottom':1})
        ...
        for index,so in enumerate(so_data):
            ...
            if so.stock_move:
                ...
                for ind,line in enumerate(so.stock_move):
                    merge_line = 5
                    sheet.merge_range(row, 0, row + merge_line, 0, line.stock_index if line.stock_index else '' , format_lines_13) 
                    sheet.merge_range(row, 1, row + merge_line, 1, line.stock_move_line_name_excel if line.stock_move_line_name_excel else '', format_lines_12_left) 
                    
                    sheet.merge_range(row, 2, row + merge_line, 3, line.product_number_and_size if line.product_number_and_size else '', format_lines_13_left) 
                    ...
                    sheet.merge_range(row, 11, row + merge_line, 11, _('有') if line.sale_line_instruction_status else '', format_lines_13) 
                    sheet.merge_range(row, 12, row + merge_line, 12, _('有'), format_lines_13) 
                    sheet.merge_range(row, 13, row + merge_line, 13, _('無'), format_lines_13) 
