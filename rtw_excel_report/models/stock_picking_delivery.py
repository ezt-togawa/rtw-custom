...
class StockPickingDelivery(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, so_data):
        ...
        for index,so in enumerate(so_data):
            ...
            header_parts = []
            if so.sale_id.name:
                header_parts.append("No. " + so.sale_id.name)
            ...
            sheet.merge_range(2, 13, 9, 14, so.sale_id.sale_order_hr_employee if so.sale_id.sale_order_hr_employee else '' , format_address) 
            ...
            if so.stock_move:
                ...
                for ind,line in enumerate(so.stock_move):
                    merge_line = 5
                    sheet.merge_range(row, 0, row + merge_line, 0, line.stock_index if line.stock_index else '' , format_lines_no) 
                    ...
                    sheet.merge_range(row, 11, row + merge_line, 11, '{0:,.2f}'.format(line.product_id.sai).rstrip('0').rstrip('.') if line.product_id.sai else '', format_lines_14)
