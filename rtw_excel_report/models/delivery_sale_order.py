...
class ReportMrpExcel(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, so_data):
        ...
        format_lines_note = workbook.add_format({'align': 'left','valign': 'vcenter', 'text_wrap':True,'font_name': font_name,'font_size':13,'bottom':1})
        format_lines_section= workbook.add_format({'align': 'left','valign': 'vcenter', 'text_wrap':True,'font_name': font_name,'font_size':13,'bg_color':'#e9ecef','bottom':1})
        
        format_lines_13_left = workbook.add_format({'align': 'left','valign': 'vcenter', 'text_wrap':True,'font_name': font_name,'font_size':13,'bottom':1})
        ...
        for index,so in enumerate(so_data):
            ...
            sheet.write(12, 12, "搬入費用：", format_text) 
            ...
            if so.order_line:
                ...
                for ind,line in enumerate(so.order_line.filtered(lambda x: not x.is_pack_outside)):
                    if line.display_type == 'line_note':
                        ...
                        merge_line = 5
                        sheet.merge_range(row, 0, row + merge_line, 0, line.sale_order_index if line.sale_order_index else '' , format_lines_13) 
                        sheet.merge_range(row, 1, row + merge_line, 1, line.sale_order_line_name_excel if line.sale_order_line_name_excel else '', format_lines_13_left) 
                        
                        sheet.merge_range(row, 2, row + merge_line, 3, line.sale_order_number_and_size if line.sale_order_number_and_size else '', format_lines_13_left) 
                        ...
                        sheet.merge_range(row, 9, row + merge_line, 9, '{0:,.0f}'.format(line.sale_line_calculate_packages)if line.sale_line_calculate_packages else '', format_lines_13) 
                        sheet.merge_range(row, 10, row + merge_line, 10, '{0:,.0f}'.format(line.product_id.sai) if line.product_id.sai else '', format_lines_13) 
                        sheet.merge_range(row, 11, row + merge_line, 11, '有' if line.instruction_status else '', format_lines_13) 
                        sheet.merge_range(row, 12, row + merge_line, 12, '有', format_lines_13) 
                        sheet.merge_range(row, 13, row + merge_line, 13, '無', format_lines_13) 
                        sheet.merge_range(row, 14, row + merge_line, 14, '', format_lines_13) 
