...
class ReportMrpExcel(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, so_data):
        ...
        img_ritzwell = img_ritzwell.resize((197, 40))
        ...
        for index,so in enumerate(so_data):
            ...
            sheet.set_header('', margin=margin_header)   
            sheet.set_footer('', margin=margin_footer) 
            ...
            sheet.insert_image(1, 8, "logo2", {'image_data': img_io_ritzwell})
            
            # y,x
            sheet.write(1, 1, "定価御見積書", format_sheet_title) 
            ...
            sheet.write(2,4, "納品希望⽇：", format_text_right) 
            sheet.write(3,4, "製作⽇数：", format_text_right) 
            sheet.write(4,4, "発注期限：", format_text_right) 
            sheet.write(5,4, "納品場所：", format_text_right) 
            sheet.write(6,4, "⽀払条件：", format_text_right) 
            sheet.write(8,4, "⾒積有効期限：", format_text_right) 
            sheet.write(9,4, "備考：", format_text_right) 
            
            sheet.write(2,5, so.preferred_delivery_period if so.preferred_delivery_period else '', format_text) 
            sheet.write(3,5, so.workdays if so.workdays else '', format_text) 
            ...
            sheet.merge_range(2,8,8,12, so.sale_order_hr_employee if so.sale_order_hr_employee else '' , format_address) 

            sheet.write(12, 11,'販売価格合計' , format_text_right) 
            sheet.write(13, 11, '値引' , format_text_right) 
            sheet.write(14, 11, '合計', format_text_right) 
            
            sheet.write(12, 12, so.sale_order_total_list_price if so.sale_order_total_list_price else '', format_text_right) 
            sheet.write(13, 12,so.sale_order_total_discount if so.sale_order_total_discount else '' , format_text_right) 
            sheet.write(14, 12,so.sale_order_amount_untaxed2 if so.sale_order_amount_untaxed2 else '' , format_text_right) 
            
            
            sheet.write(15, 12,so.name if so.name else '' , format_text_right) 
            ...
            sheet.merge_range(16, 9,16,11, "定価", format_table)
            ...
            if so.order_line:
                ...
                for ind,line in enumerate(so.order_line):
                    ...
                    if line.display_type == 'line_note':
                        sheet.merge_range(row,0,row + merge_line,12, "=data!A" + str(ind * 1 + 1) , format_lines_note) 
                        sheet_data.write(ind,0, line.name if line.name else '', format_lines_note) 
                    elif line.display_type == 'line_section':
                        sheet.merge_range(row,0,row + merge_line,12, "=data!B" + str(ind * 1 + 1) , format_lines_section) 
