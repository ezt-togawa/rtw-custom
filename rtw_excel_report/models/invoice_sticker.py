...
class productLabelSticker(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for stock_picking in lines :
            ...
            #                 [("id", "=", stock_moves.sale_line_id.ids)]
            #     )
            ...
            if stock_moves:
                for index,line in enumerate(stock_moves):
                    ...
                    # sheet_main= workbook.add_worksheet(f"{stock_picking.name}_{index}")
                    
                    sheet_name = f"{prod_name}_{index}"
                    sheet_name=sheet_name.replace(" ", "")
                    sheet_main= workbook.add_worksheet(sheet_name)
                    # sheet_main.set_row(4, 15)  

                    # data_name=stock_picking.name.strip().split("/")
                    # #  data_name[0]=WH     data_name[1]=OUT   data_name[2]=00030_0
                    # sheet_data_name= data_name[0]+"/"+data_name[1]+"/"+data_name[2]
                    # sheet_data= workbook.add_worksheet(f"data{sheet_data_name}_{index}")
                    ...
                    for count in range(2):
                        ...
                        sheet_data.write(count, 8, stock_picking.partner_id.commercial_company_name, merge_format) # depo name
