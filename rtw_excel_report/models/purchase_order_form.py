                ...
                for ind,line in enumerate(so.order_line.filtered(lambda x: not x.is_pack_outside)):
                    ...
                    if line.display_type == 'line_note':
                        ...
                        if line.is_tax_excluded_product:
                            ...
                            sheet.merge_range(row, 9, row + merge_line, 9, line.product_uom_qty or 0, format_lines_14)
                            sheet.merge_range(row, 10, row + merge_line, 10, line.price_unit or 0, format_lines_13)
                            sheet.merge_range(row, 11, row + merge_line, 11, line.call_rate or 0, format_lines_14)
