...
class ReportMrpExcel(models.AbstractModel):
    """
        【単価御見積書】Excelレポート出力ロジック
        レポートID: rtw_excel_report.unit_price_quotation_xls
        用途: 販売オーダーより単価御見積書のExcelを作成する
    """
    _name = 'report.rtw_excel_report.unit_price_quotation_xls'
    ...
    def generate_xlsx_report(self, workbook, data, so_data):
        ...
        format_text_right = workbook.add_format({'align': 'right', 'font_name': font_name, 'font_size':11.25})
        format_note = workbook.add_format({'align': 'left', 'valign': 'top', 'font_name': font_name, 'font_size':10, 'shrink':1})
        ...
        for index,so in enumerate(so_data):
            ...
            # y,x
            sheet.write(1, 1, _("単価御見積書"), format_sheet_title)
            ...
            if so.order_line:
                ...
                # 改ページ位置(0-indexed行)は report_quotation.py の実測値をそのまま使用。
                # 14行目(消費税は含まれておりません)と品目表の間に空白行を1行入れたことで
                # 品目表開始行が report_quotation.py と同じ(0-idx16)になったため、シフト不要。
